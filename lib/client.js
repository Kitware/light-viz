...
export function saveThumbnail() {
  call('light.viz.dataset.thumbnail.save', []).then(
    onReady(),
...
export function getBlockStructure(callback) {
  call('light.viz.dataset.getblockstructure').then((list) => {
    structure = list;
    onReady();
    callback(structure);
...
export function setForegroundColor(color) {
  call('light.viz.foreground.color', [color]).then(
    onReady(),
...
export function setBackgroundColor(color) {
  call('light.viz.background.color', [color]).then(
    onReady(),
...
export function resetCamera() {
  call('viewport.camera.reset', [-1]).then(
    onReady,
    onError.bind(undefined, 'light.viz.camera.reset')
  );
}
...
export function updateDatasetOpacity(opacity) {
  call('light.viz.dataset.opacity', [opacity]).then(
    onReady(),
    onError.bind(undefined, 'light.viz.dataset.opacity')
  );
}

export function updateBlockVisibility(blockVisibilities) {
  call('light.viz.dataset.setblock.visibility', [blockVisibilities]).then(
    onReady(),
    onError.bind(undefined, 'light.viz.dataset.setblock.visibility')
  );
}
...
export function loadDataset(dsName, callback) {
  if (activeDataset !== dsName || !lastDS) {
    ...
    call('light.viz.dataset.load', [activeDataset]).then((ds) => {
      onReady();
      lastDS = ds;
      callback(ds);
...
export function updateClipPosition({ xPosition, yPosition, zPosition }) {
  call('light.viz.clip.position', [xPosition, yPosition, zPosition]).then(
    onReady,
...
export function updateClipInsideOut(x, y, z) {
  call('light.viz.clip.insideout', [x, y, z]).then(
    onReady,
    onError.bind(undefined, 'light.viz.clip.insideout')
  );
}

export function updateClipBoxPosition({ xPosition, yPosition, zPosition }) {
  call('light.viz.clip.box.position', [xPosition, yPosition, zPosition]).then(
    onReady,
...
export function enableClipBox(show) {
  call('light.viz.clip.box.show', [show]).then(
    onReady,
...
export function updateContourValues(values) {
  call('light.viz.contour.values', [values]).then(
    onReady,
...
export function updateContourBy(field) {
  call('light.viz.contour.by', [field]).then(
    onReady,
...
export function updateSlicePosition({ xPosition, yPosition, zPosition }) {
  call('light.viz.slice.position', [xPosition, yPosition, zPosition]).then(
    onReady,
    onError.bind(undefined, 'light.viz.slice.position')
  );
}

export function updateSlicesVisible(x, y, z) {
  call('light.viz.slice.visibility', [x, y, z]).then(
    onReady,
    onError.bind(undefined, 'light.viz.slice.visibility')
  );
}
...
export function updateMultiSliceValues(positions) {
  call('light.viz.mslice.positions', [positions]).then(
    onReady,
    onError.bind(undefined, 'light.viz.mslice.positions')
  );
}

export function updateMultiSliceAxis(axis) {
  call('light.viz.mslice.normal', [axis]).then(
    onReady,
    onError.bind(undefined, 'light.viz.mslice.normal')
  );
}
...
export function updateStreamlineSeedPoint({ xPosition, yPosition, zPosition }) {
  call('light.viz.streamline.position', [xPosition, yPosition, zPosition]).then(
    onReady,
    onError.bind(undefined, 'light.viz.streamline.position')
  );
}

export function updateStreamlineVector(vector) {
  call('light.viz.streamline.vector', [vector]).then(
    onReady,
    onError.bind(undefined, 'light.viz.streamline.vector')
  );
}

export function updateStreamlineSeedRadius(radius) {
  call('light.viz.streamline.radius', [radius]).then(
    onReady,
    onError.bind(undefined, 'light.viz.streamline.radius')
  );
}

export function updateStreamlineNumPoints(numPoints) {
  call('light.viz.streamline.numpoints', [numPoints]).then(
    onReady,
    onError.bind(undefined, 'light.viz.streamline.numpoints')
  );
}

export function showStreamlineSeed(enableSeed) {
  call('light.viz.streamline.seed.show', [enableSeed]).then(
    onReady,
    onError.bind(undefined, 'light.viz.streamline.seed.show')
  );
}

export function updateStreamlineSeed(position, radius) {
  call('light.viz.streamline.seed.update', [position, radius]).then(
    onReady,
    onError.bind(undefined, 'light.viz.streamline.seed.update')
  );
}
...
export function setColorMapPreset(arrayName, preset) {
  call('light.viz.colormap.setpreset', [arrayName, preset]).then(
    onReady(),
    onError.bind(undefined, 'light.viz.colormap.setpreset')
  );
}

export function setColorMapRange(arrayName, range) {
  call('light.viz.colormap.setrange', [arrayName, range]).then(
    onReady(),
    onError.bind(undefined, 'light.viz.colormap.setrange')
  );
}

export function setColorMapRangeToDataRange(arrayName, callback) {
  call('light.viz.colormap.rescale.todatarange', [arrayName]).then((result) => {
    callback(result);
  }, onError.bind(undefined, 'light.viz.colormap.rescale.todatarange'));
}

export function updateOpacityMap(arrayName, controlPoints) {
  call('light.viz.opacitymap.set', [arrayName, controlPoints]).then(
    onReady(),
    onError.bind(undefined, 'light.viz.opacitymap.set')
  );
}

export function getColorMap(arrayName, callback) {
  call('light.viz.colormap.get', [arrayName]).then((result) => {
...
export function getOpacityMap(arrayName, callback) {
  call('light.viz.opacitymap.get', [arrayName]).then((points) => {
    callback(points);
...
export function updateUseClip(type, useClip) {
  ...
  call(name, [useClip]).then(onReady, onError.bind(undefined, name));
}
...
export function updateRepresentation(type, mode) {
  ...
  call(name, [mode]).then(onReady, onError.bind(undefined, name));
}
