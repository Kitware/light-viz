...
export default React.createClass({
    ...
    componentWillMount() {
        ...
        this.setState({ thumbnailIndices: indices });
    },
    ...
    componentWillReceiveProps(nextProps) {
        if (!equals(nextProps.list, this.props.list)) {
            this.setState({ thumbnailIndices: []});
        }
    },
    ...
    tick() {
        ...
        this.setState({ thumbnailIndices: newIndices });
    },
        ...
        return (<div className="ThumbnailList">
                  { this.props.list.map(function(ds,index) {
                    return <Link to={"/view/" + ds.name} key={index} >
                             <img className="ThumbnailList_Thumbnail" data-index={indices[index]} data-name={ds.name}/>
