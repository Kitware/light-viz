...
export default React.createClass({
  ...
  tick(e) {
    ...
    const newIdxScale = (e.clientX - (targetRect.x || targetRect.left || 0)) / targetRect.width;
    const newIndices = [];
    for (let i = 0; i < this.props.list.length; ++i) {
      newIndices.push(this.state.thumbnailIndices.length > i ? this.state.thumbnailIndices[i] : 0);
    }
    if (this.state.currentThumbnail !== null && newIndices[this.state.currentThumbnail] !== undefined) {
      newIndices[this.state.currentThumbnail] = Math.floor(newIdxScale * this.props.list[this.state.currentThumbnail].thumbnails.length);
    }
    this.setState({
