...
import ForegroundBackground from '../../widgets/ForegroundBackground';
...
export default React.createClass({
  ...
  render() {
    ...
    const newModules = modules.filter(mdl => {
      if (this.state.profile.modules_included.includes(mdl.name)) {
        return true;
      }
      if (this.state.profile.modules_included.length === 0 &&
        !this.state.profile.modules_excluded.includes(mdl.name)) {
        return true;
      }
      return false;
    });
