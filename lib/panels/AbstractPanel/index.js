  ...
  updateRepresentation(name, repr) {
    ...
    if (repr === 'Volume') { // Volume rendering
      if (this.colorBy.state.value === '__SOLID__') {
        this.colorBy.setState({ value: this.props.dataset.data.arrays[0].name });
  ...
  render() {
    ...
    const inputSelector = (
      <div className={style.inputSelector} onClick={this.updateInputTo}>
        <span className={style.visibleModuleName} style={{ position: 'relative', top: '-4px' }}>{'<'}</span>
        <SvgIcon
