...
const blockVisibilityIcons = [
  style.blockIsHiddenIcon,
  style.blockIsShownIcon,
  style.blockIsPartiallyHiddenIcon,
];
...
export default class DatasetPanel extends React.Component {
  constructor(props) {
    ...
    this.getBlockVisibilityState = this.getBlockVisibilityState.bind(this);
    this.createBlocksTree = this.createBlocksTree.bind(this);
    this.needsApply = this.needsApply.bind(this);
    this.updateState = this.updateState.bind(this);
    this.updateOpacity = this.updateOpacity.bind(this);
    this.updateBlockStructure = this.updateBlockStructure.bind(this);
    this.toggleCollapsed = this.toggleCollapsed.bind(this);
    this.recursiveSetVisibility = this.recursiveSetVisibility.bind(this);
    this.toggleBlockShown = this.toggleBlockShown.bind(this);
  }
  ...
  componentDidMount() {
    getState('dataset', this, this.modulePanel);
  }
    ...
    const collapseIcon =
      block.children.length > 0
        ? this.state.collapsed[block.flatindex]
          ? style.blockIsCollapsedIcon
          : style.blockIsExpandedIcon
        : style.blockHasNoChildrenIcon;
    const cName = this.state.collapsed[block.flatindex]
      ? style.childBlocksDivCollapsed
    ...
    return (
      <div className={cName} key={index}>
        <span className={style.blockSpan}>
          ...
          >
            <i className={blockVisibilityIcons[visibilityState]} />
          </span>
        </span>
        {block.children.length > 0
          ? block.children.map(this.createBlocksTree)
          : null}
      </div>
    );
  }
  ...
  needsApply() {
    return this.state.opacity === this.state.oldOpacity;
  }
        ...
        >
          <span className={style.blockSpan}>
            ...
            >
              <i
                className={
                  blockVisibilityIcons[this.getBlockVisibilityState(0)]
                }
              />
            </span>
          </span>
