...
export default React.createClass({
    ...
    updateBlockStructure(structure) {
        const showBlock = [], collapsed = [], parentBlock = [];
        if (structure.length > 0) {
            ...
            const toProcess = [ { flatindex: 0, children: structure } ];
        ...
        this.setState({blocks: structure, collapsed, showBlock});
    },
    ...
    createBlocksTree(block, index) {
      ...
      const cName = this.state.collapsed[block.flatindex] ? style.childBlocksDivCollapsed : style.childBlocksDiv;
      return <div className={ cName } key={index}>
               <span className={ style.blockSpan }>
                 <i className={ collapseIcon} onClick={ this.toggleCollapsed.bind(this,block.flatindex)}/>
                 {block.name}
                 <i className={ this.state.showBlock[block.flatindex] ?
                   style.blockIsShownIcon : style.blockIsHiddenIcon}
                   onClick={ this.toggleBlockShown.bind(this,block.flatindex)}/>
               </span>
    ...
    toggleBlockShown(index) {
        ...
        if (shown) {
            ...
            while (showParent && !showBlock[pidx]) {
                ...
                if (showParent) {
                    showBlock[pidx] = true;
                    pidx = pidx === this.blockParents[pidx];
                }
            }
        }
    ...
    render() {
        return (<AbstractPanel ref={(c) => { this._modulePanel = c}}
                  ...
                  <div className={ this.state.collapsed[0] ? style.childBlocksDivCollapsed : style.childBlocksDiv }>
                    <span className={ style.blockSpan }>
                      <i className={ this.state.collapsed[0] ? style.blockIsCollapsedIcon :
                                                               style.blockIsExpandedIcon }
                         onClick={this.toggleCollapsed.bind(this,0)}/>
                      Blocks
                      <i className={ this.state.showBlock[0] ? style.blockIsShownIcon 
                                                             : style.blockIsHiddenIcon }
                         onClick={ this.toggleBlockShown.bind(this,0)}/>
                   </span>
                    {
