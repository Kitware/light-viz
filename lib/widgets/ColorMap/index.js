...
export default React.createClass({
  ...
  updateActiveArray(arrayLabel) {
    let currentArray = -1;
    this.props.dataset.data.arrays.forEach((item, index) => {
      if (currentArray !== -1) {
        return;
      }
      if (item.label === arrayLabel) {
        currentArray = index;
        return;
      }
    });
  ...
  render() {
    ...
    return (
      <CollapsibleWidget
        ...
        subtitle={
          <DropDownWidget
            field={this.props.dataset.data.arrays[this.state.currentArray].label}
            fields={this.props.dataset.data.arrays.map(item => item.label)}
