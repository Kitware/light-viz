...
export default React.createClass({
  ...
  render() {
     ...
     */
    if (
      this.props.independentVisibilityPanels.indexOf(this.props.panelId) !== -1
    ) {
      if (!panelVisible) {
      ...
      query.visible = !panelVisible
        ? [this.props.panelId].concat(visibleIndependentPanels).join('|')
    ...
    const rootElementClass = this.props.hidden
